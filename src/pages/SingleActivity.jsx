            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 rounded-[2rem] overflow-hidden h-[300px] md:h-[500px] mb-12 relative z-0">
                    <div className="h-full cursor-pointer group" onClick={() => openModal(0)}>
                        <img src={data?.images?.[0]?.url || "https://images.unsplash.com/photo-1544551763-46a013bb70d5?auto=format&fit=crop&w=800&q=60"} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" alt="Main" />
                    </div>
                    <div className="hidden md:grid grid-rows-2 gap-3 h-full">
                        <div className="cursor-pointer overflow-hidden group" onClick={() => openModal(1)}>
                            <img src={data?.images?.[1]?.url || data?.images?.[0]?.url || "https://images.unsplash.com/photo-1551632811-561f32a74c0c?auto=format&fit=crop&w=800&q=60"} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" alt="Secondary 1" />
                        </div>
                        <div className="cursor-pointer overflow-hidden group" onClick={() => openModal(2)}>
                            <img src={data?.images?.[2]?.url || data?.images?.[0]?.url || "https://images.unsplash.com/photo-1534149957077-03b070440268?auto=format&fit=crop&w=800&q=60"} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" alt="Secondary 2" />
                        </div>
                    </div>
                </div>
                    ...
                    <div className="relative">
                        <div className="sticky top-28 p-8 border border-slate-200 rounded-3xl shadow-lg bg-white z-10">
                            <div className="flex items-end gap-1 mb-6">
                                <span className="text-3xl font-black text-slate-900">₹{data?.price?.toLocaleString('en-IN') || 0}</span>
                                <span className="text-base text-slate-500 font-medium mb-1">/ person</span>
                            </div>
                    ...
                    <div className="h-[450px] w-full rounded-3xl overflow-hidden shadow-inner border border-slate-200">
                        <div className="relative w-full h-full z-0">
                            {lat && lng ? (
                                ...
                                <div className="w-full h-full bg-slate-100 flex items-center justify-center text-slate-400">Map not available</div>
                            )}
                        </div>
                    </div>
