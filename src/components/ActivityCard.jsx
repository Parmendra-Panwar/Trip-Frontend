...
const ActivityCard = ({ data }) => {
    ...
    const { isFavorite, toggleFavorite } = useFavorites('activity');
    const primaryImage = (data.images && data.images.length > 0 && data.images[0])
        ? data.images[0].url
        : (data.image?.url || data.image || 'https://images.unsplash.com/photo-1551632811-561f32a74c0c?auto=format&fit=crop&w=800&q=60');
        ...
        <Link to={`/activity/${id}`} className="group cursor-pointer block">
            <div className="relative aspect-[20/19] overflow-hidden rounded-[1.25rem] bg-[#EBEBEB] mb-3">
                ...
                <button
                    ...
                    className="absolute top-3 right-3 p-2 cursor-pointer group-active:scale-90 transition-transform z-10"
                >
                    <svg
                        className={`w-[28px] h-[28px] drop-shadow-md transition-colors ${isFavorite(id) ? 'text-red-500 fill-red-500' : 'text-white/80 hover:text-white'}`}
            ...
            <div className="flex flex-col">
                <div className="flex justify-between items-start gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0 mt-[2px]">
                        <svg className="w-3.5 h-3.5 text-[#222222]" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M12 .587l3.668 7.431 8.2 1.151-6.064 5.828 1.58 8.163L12 18.896l-7.384 3.882 1.58-8.163L.132 9.169l8.2-1.151z" />
                        </svg>
                ...
                <div className="mt-1.5 flex items-baseline gap-1">
                    ...
                    <span className="text-[#222222] font-[400] text-[15px]">{data.duration ? `/ ${data.duration}` : '/ session'}</span>
                </div>
            </div>
        </Link>
    );
